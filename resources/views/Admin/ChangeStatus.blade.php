...
<div class="title1" style="color:white;">
    <div class="title-text" style="text-align:center;">
        <textarea  placeholder="Type your title here" class="Title-text"  style="cursor: not-allowed">{{$res[0]->title}}</textarea>
    ...
    <div class="code" >
        <div class="NormalCode">
            ...
            <textarea onkeyup="textAreaAdjust(this);" style="overflow:hidden"  placeholder="type category here"  class="category">{{$res[0]->category}}</textarea>

            <p class="PreRenderedMarkDown"># Description</p>
            <textarea onkeyup="textAreaAdjust(this);" style="overflow:hidden" class="code1" placeholder="Type your descrption here">{{$res[0]->description}}</textarea>
        </div>
        <div></div>
        <div class="MarkDown1">
...
<div style="margin-top:5px;float:right">
    <button class="btn1" onclick="Register()"><b>Approve Question</b> </button>
</div>
<br>
<br>
       ...
       async function Register() 
       {
            ...
            if(desc.length>0){
                axios.post('/MarkDown',{markdown:desc}).then((response)=>{          
                    desc = response
                    axios.post("/update",{title:title,category:category,difficulty:difficulty,desc:desc,type:"approve",email:"{!! $email !!}"}).then((response)=>{
